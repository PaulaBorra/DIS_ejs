...
public class DataService {
    ...
    public String addHospital(Hospital hospital) {
        try {
            String hospitalJson = gson.toJson(hospital);
            HttpRequest request = HttpRequest.newBuilder()
                    .uri(URI.create(BASE_URL + "/add-hospital"))
                    .POST(HttpRequest.BodyPublishers.ofString(hospitalJson))
                    .header("Content-Type", "application/json")
                    .build();
            ...
            logger.error("Error adding hospital: {}", hospital.getHospitalName(), e);
            throw new RuntimeException("Error adding hospital: " + e.getMessage(), e);
        }
    }
}